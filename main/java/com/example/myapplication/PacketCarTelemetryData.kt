...
fun parsePacketCarTelemetryData(data: ByteArray, length: Int): PacketCarTelemetryData {
    ...
    val carTelemetryData = Array(22) {
        ...
        val throttle = buffer.float                           // CAMBIATO: float invece di uint8
        val steer = buffer.float                              // CAMBIATO: float invece di int8
        val brake = buffer.float                              // CAMBIATO: float invece di uint8
        ...
        // Surface type for each wheel [RL, RR, FL, FR]
        buffer.position(buffer.position() + 4)  // skip 4 bytes (surfaceType array)
    ...
    val mfdPanelIndex = buffer.get().toInt() and 0xFF
    val mfdPanelIndexSecondaryPlayer = buffer.get().toInt() and 0xFF
    val suggestedGear = buffer.get().toInt()
