...
class UdpReceiver(
    ...
    //  - Traccia quale pilota ha fatto ogni best sector
    private var bestS1DriverIndex = -1
    private var bestS2DriverIndex = -1
    private var bestS3DriverIndex = -1

    //  - Traccia la sessione corrente per resettare i sectors
    ...
    private var isFlashbackActive = false

    // ⭐ NUOVO - Stato connessione
    private val _isReceivingData = MutableStateFlow(false)
    val isReceivingData: StateFlow<Boolean> = _isReceivingData

    private val _isInSession = MutableStateFlow(false)  // ⭐ NUOVO
    val isInSession: StateFlow<Boolean> = _isInSession

    private var lastPacketTime = 0L
    private var lastSessionPacketTime = 0L  // ⭐ NUOVO
    ...
    fun start() {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                while (true) {
                    ...
                    // ⭐ AGGIUNTO - Aggiorna timestamp quando ricevi un pacchetto
                    ...
                    when (header.packetId) {
                        ...
                        1 -> { // Session Data
                            ...
                            // ⭐ AGGIUNTO - Aggiorna timestamp sessione
                            lastSessionPacketTime = System.currentTimeMillis()
                        ...
                        3 -> { // Event Packet
                            ...
                            val eventCode = eventData.eventStringCode

                            Log.d("UDP_EVENT", "🎮 Event received: $eventCode")

                            when (eventCode) {
                                "FLBK" -> { // ⭐ Flashback - NON resettare
                                    Log.d("UDP_EVENT", "🔄 FLASHBACK detected - skipping reset")
                                ...
                                "SSTA" -> {
                                    if (!isFlashbackActive) {  // ⭐ Reset solo se NON è flashback
                                        resetTelemetry()
                                    }
                                }
                        ...
                        6 -> { // Car Telemetry
                            Log.d("UDP_TELEMETRY", "📦 Telemetry packet received")
                            ...
                            val speed = myCarData.speed.toInt()

                            val tyreFLTemp= myCarData.tyresSurfaceTemperature[2]
                            val tyreFRTemp= myCarData.tyresSurfaceTemperature[3]
                            val tyreRLTemp= myCarData.tyresSurfaceTemperature[0]
                            val tyreRRTemp= myCarData.tyresSurfaceTemperature[1]
                            Log.d("UDP_TELEMETRY", "Speed: $speed km/h, Temp FL: ${tyreFLTemp}°C")
                            ...
                            _telemetryState.value = _telemetryState.value.copy(
                                ...
                                // ⭐ AGGIUNGI le temperature
                                tyreFLTemp = tyreFLTemp,
                                tyreFRTemp = tyreFRTemp,
                                tyreRLTemp = tyreRLTemp,
                                tyreRRTemp = tyreRRTemp
                            )
        ...
        // ⭐ NUOVO - Thread separato che controlla il timeout
        CoroutineScope(Dispatchers.IO).launch {
            while (true) {
                delay(2000) // Controlla ogni 2 secondi

                // Se non riceviamo dati da più di 3 secondi
                if (System.currentTimeMillis() - lastPacketTime > 3000 && lastPacketTime > 0) {
                    _isReceivingData.value = false
                }

                // ⭐ NUOVO - Se non riceviamo session data da più di 5 secondi
                if (System.currentTimeMillis() - lastSessionPacketTime > 5000 && lastSessionPacketTime > 0) {
